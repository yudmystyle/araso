...
public class Demo2Controller : MonoBehaviour
{
    ...
    [Tooltip("The current level display text")]
    public Text LevelText;
    [Tooltip("The level progress bar")]
    public Slider ProgressBar;
    [Tooltip("The correct answers remaining to the next level display text")]
    public Text CorrectAnswersLeft;
    ...
    [Tooltip("The category name component")]
    public Text CategoryName;
    [Tooltip("The category display component")]
    public Image CategoryDisplay;
    ...
    [Tooltip("The category pick button")]
    public GameObject PickButton;
    [Tooltip("The retry button")]
    public GameObject RetryButton;
    [Tooltip("The play button")]
    public GameObject PlayButton;
    [Tooltip("The retry ad panel")]
    public GameObject RetryAdPanel;
    ...
    private static string ShareImageName = "OriginalImg";
    //Answer of currently displayed question
    private int CorrectAnswer = 0;
    //A count of the correctly answered questions
    private int CorrectlyAnswered = 3;
    //Answer text holders
    public Text[] AnswerA, AnswerB, AnswerC, AnswerD = new Text[2];
    ...
    //Game category array
    private Demo2XmlDownloader[] GameCategories = new Demo2XmlDownloader[0];
    ...
    //Integers we will use when getting the category
    private int a = 0;
    private int b = 0;
    private int c = 0;
    private int r = 0;
    ...
    void Start()
    {
        ...
        GameCategories = new Demo2XmlDownloader[Count];

        GameCategories = FindObjectsOfType(typeof(Demo2XmlDownloader)) as Demo2XmlDownloader[];
    ...
    void UpdateProfile()
    {
        ...
        CorrectlyAnswered = PlayerPrefs.GetInt("CorrectAnswers", 0);
        //Display the current players level
        LevelText.text = "Level " + CurrentLevel.ToString();

        int Result = (CurrentLevel * 5) - CorrectlyAnswered;
        //Displays the answers left to the next level
        if (Result <= 1)
            CorrectAnswersLeft.text = Result.ToString() + " correct answer to the next level";
        else
            CorrectAnswersLeft.text = Result.ToString() + " correct answers to the next level";

        ProgressBar.maxValue = CurrentLevel * 5;
        ProgressBar.value = CorrectlyAnswered;

        for (int i = 0; i < GameCategories.Length; i++)
            GameCategories[i].PerformanceText.text = PlayerPrefs.GetFloat(GameCategories[i].PerformancePref, 0).ToString("0 '%'");
    }
    ...
    public void StartGame()
    {
        ...
        //Enable the Category Canvas and hide the Menu Canvas
        MenuCanvas.SetActive(false);
        CategoryCanvas.SetActive(true);

        CategoryDisplay.sprite = GameCategories[0].CategoryImage;
        CategoryName.text = GameCategories[0].Name;
        ...
        RetryAdPanel.SetActive(false);

        //Enable the category pick button and disable the rest
    ...
    public void HideCategory()
    {
        ...
        //Hide the Category Canvas and show the Menu Canvas
        MenuCanvas.SetActive(true);
        CategoryCanvas.SetActive(false);
    ...
    public void RandomCategory()
    {
        ...
        //Disable the category buttons and disable the rest
        StartCoroutine(CategoryLoop());
        PickButton.SetActive(false);
        PlayButton.SetActive(false);
        RetryButton.SetActive(false);
    ...
    //We will use this to play a short animation of the categories shuffling and then display a specific category after a short while
    IEnumerator CategoryLoop()
    {
        CategoryShow = false;

        Playing = true;

        CategoryDisplay.sprite = GameCategories[a].CategoryImage;
        CategoryName.text = GameCategories[a].Name;
        a++;
        if (a > GameCategories.Length - 1)
        {
            a = 0;
            b++;
        }
        ...
        {
            r = UnityEngine.Random.Range(0, GameCategories.Length);
            while (r == c)
                r = UnityEngine.Random.Range(0, GameCategories.Length);

            c = r;
            CategoryName.text = GameCategories[r].Name;
            CategoryDisplay.sprite = GameCategories[r].CategoryImage;
            PlayButton.SetActive(true);
    ...
    public void ShowRetryAdPanel()
    {
#if (USE_UNITYADS)
        RetryAdPanel.SetActive(true);
#else
        RandomCategory();
#endif
    }
    ...
    public void HideRetryAdPanel()
    {
        RetryAdPanel.SetActive(false);
    }

    public void ShowRetryVideoAd()
    {
        RetryAdPanel.SetActive(false);
        var Retryoptions = new ShowOptions { resultCallback = HandleShowAdResult };
        Advertisement.Show(Retryoptions);
    }
    ...
    {
        RandomOrder = GameCategories[r].ShuffleQuestions;
        ...
        string Offlinepath = "XML/" + GameCategories[r].OfflinePath;

        if (GameCategories[r].Mode == DownloadMode.Online)
            StartCoroutine(DownlaodXML(GameCategories[r].OnlinePath));
        else if (GameCategories[r].Mode == DownloadMode.Offline)
            StartCoroutine(LoadQuestions(Offlinepath));
        else
            StartCoroutine(DownlaodXML(GameCategories[r].OnlinePath, Offlinepath, true));
    ...
    void ShuffleAnswers()
    {
        ...
        for (int index = 0; index < AnswersAvailable.Count; index++)
        {
    ...
    public void Answers(int ButtonIndex)
    {
        ...
        {
            //Update the category performance for the currently loaded question
        ...
        {
            //Update the category performance
            CategoryPerformance(0);

            //Set the user input to red to display a fail and show the correct answer
    ...
    void SetScore()
    {
        ...
        //Get the correctly answered and current level ints
        CorrectlyAnswered = PlayerPrefs.GetInt("CorrectAnswers", 0);
        CurrentLevel = PlayerPrefs.GetInt("Level", 1);
        ...
        {
            PlayerPrefs.SetInt("CorrectAnswers", CorrectlyAnswered);
        }
    ...
    //Update the category performance
    void CategoryPerformance(int value)
    {
        //The questions asked so far in the cars category
        float questions = PlayerPrefs.GetFloat(GameCategories[r].QuestionPref, 0);
        //The correct answered questions so far in the cars category
        float correct = PlayerPrefs.GetFloat(GameCategories[r].AnswerPref, 0);

        questions += 1;
        correct += value;

        PlayerPrefs.SetFloat(GameCategories[r].QuestionPref, questions);
        PlayerPrefs.SetFloat(GameCategories[r].AnswerPref, correct);

        PlayerPrefs.SetFloat(GameCategories[r].PerformancePref, (correct / questions) * 100);
    ...
    {
#if (SUPPORTED_PLATFORM || UNITY_EDITOR)
        //Hide the continue and review button
    ...
    void checkLives()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                FinalScore.text = "Genius!";
                //Display scores
                CurHighscore.text = "\nScore: " + score.ToString("0") + "\n" +
                "Highscore: " + PlayerPrefs.GetFloat("Highscore", 0).ToString("0");
            }
        }
    ...
    private IEnumerator NextQuestion()
    {
        ...
        for (int q = 0; q < Questions[0].Answer.Length; q++)
            AnswersAvailable.Add(Questions[0].Answer[q]);

        //Shuffle the answers
        ShuffleAnswers();

        if (!OnlineMode && Questions[0].Image.Length > 1)
        {
            ImageDispay.sprite = Resources.Load<Sprite>("Images/" + Questions[0].Image);
            ImageDispaySc.sprite = Resources.Load<Sprite>("Images/" + Questions[0].Image);
            TextOnly = false;
        }
        else if (OnlineMode && Questions[0].Image.Length > 1)
        {
        TryAgain:

            string EncodedString = CalculateMD5Hash(Questions[0].Image);
            ...
            else
            {
                ...
                while (!File.Exists(filePath))
                {
                    ...
                    {
                        if (Questions[0].Image.Equals(FaultyLinks[count]))
                        {
        ...
        //Display the question and image if any
        AltQuestionDisplay.text = Questions[0].Question;
        QuestionDisplay.text = Questions[0].Question;
        QuestionDisplaySc.text = Questions[0].Question;
    ...
    void PrepareNewGame()
    {
        ...
        //Show the category we are currently playing
        CurrentCategoryText.text = GameCategories[r].Name;
